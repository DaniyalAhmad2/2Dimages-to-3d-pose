...
| 6 | …and, inside the same script, `check_bundle_deps.py` — **once** | a DLL missing from `_internal\` still loads on this runner, which has the Visual C++ runtime installed system-wide, and only fails on a clean machine: *"Failed to load Python DLL … python312.dll. LoadLibrary: The specified module could not be found."* Run while the bundle holds only the app, before Blender lands beside it — Blender ships its own C runtime and is not ours to audit. The release workflow used to run the audit a second time for the same answer. Since build 17 the audit also refuses to class the Universal C Runtime (`ucrtbase.dll`, `api-ms-win-crt-*`) as Windows-provided: a client's Windows did not have it in working order, and `python312.dll` failed with this same dialog on a *complete* folder. The spec bundles it from the Windows SDK redist; the manifest requires it; `Diagnose.cmd` checks for it in plain cmd, because the diagnose exe is Python too. |
