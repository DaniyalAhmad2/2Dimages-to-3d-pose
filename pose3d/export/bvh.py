"""A minimal BVH reader, so an exported file can be asserted on directly.
...
orphan IK helper bone carried 42 % of the rig's height of translation (F13).
Reading the file needs no Blender, so those facts can be checked on every run.
...
It lives in the shipped package, not in `tests/`, because `tools/
check_export_fidelity.py` reads exports with it: a shipped tool must not import
the test package. `tests/bvh_util.py` re-exports everything here.
"""
...
def keyframe_rows(bvh: Bvh, n_poses: int, segment: int = SEGMENT,
                  hold: int = HOLD) -> list[tuple[int, int]]:
    """(first_row, last_row) of each captured pose in `bvh`, 0-based.

    Two schedules can produce a file: one frame per pose (the BVH/FBX default
    since Phase 3, where export frame k+1 IS photograph k) and the stepped
    stop-motion hold the mp4 still uses. Which one wrote a file is read off its
    frame count rather than assumed, so a caller can measure either without
    being told, and a file that is neither is refused instead of silently
    measured against the wrong rows.
    """
    if bvh.n_frames == n_poses:
        return [(i, i) for i in range(n_poses)]
    if bvh.n_frames == expected_frames(n_poses, segment, hold):
        return stepped_holds(n_poses, segment, hold)
    raise ValueError(
        f"{bvh.n_frames} motion rows is neither {n_poses} (one frame per "
        f"pose) nor {expected_frames(n_poses, segment, hold)} (stepped)")


def channel_overshoot(bvh: Bvh, n_poses: int, holds=None) -> np.ndarray:
    ...
    holds = stepped_holds(n_poses) if holds is None else list(holds)
    out = []
...
def similarity(src: np.ndarray, dst: np.ndarray):
    ...
    An exported file is in the armature's units and space and the app's joints
    are in metres, so a comparison between them is only meaningful after one
    similarity fit. ONE, for the whole take: fitting per frame forgives exactly
    the placement error this module exists to catch.
    """
...
def similarity_error(src: np.ndarray, dst: np.ndarray) -> np.ndarray:
    """Per-point residual after the single best similarity fit of src onto dst."""
    scale, R, t = similarity(src, dst)
    return np.linalg.norm((scale * (R @ src.T).T + t) - dst, axis=1)
